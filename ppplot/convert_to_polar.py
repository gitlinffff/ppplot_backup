...
if not args.output:
    args.output = args.input.replace('.nc', '_polar.nc')

# Users specify variables to process
...
def process_timestep(t):
    with Dataset(args.input, mode='r') as nc:
...
with Dataset(args.output, mode='w') as new_nc:
